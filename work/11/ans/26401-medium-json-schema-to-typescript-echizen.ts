...
type Prim = keyof PrimMap

type Type = Prim | 'object' | 'array'
...
type HandlePrim<T, Type extends Prim> = T extends {
  enum: any[]
}
  ? T['enum'][number]
  : PrimMap[Type]
...
type HandleObject<T> = T extends {
  properties: infer P extends Record<PropertyKey, any>
}
  ? JSONSchema2TS<P>
  : Record<string, unknown>

type HandleArray<T> = T extends {
  items: infer P
}
  ? JSONSchema2TS<P>
  : unknown[]
