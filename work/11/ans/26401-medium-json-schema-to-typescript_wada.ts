...
type JsonObject = {
  type: "object"
  properties?: Record<string, JsonItemType>
  required?: string[]
}
type JsonArray = {
  type: "array"
...
// prettier-ignore
type JSONSchema2TS<T> = T extends JsonString
  ? undefined extends T['enum'] ? string : T['enum'] extends any[] ? T['enum'][number] : never
  : T extends JsonNumber
  ? undefined extends T['enum'] ? number : T['enum'] extends any[] ? T['enum'][number] : never
  : T extends JsonBoolean
  ? undefined extends T['enum'] ? boolean : T['enum'] extends any[] ? T['enum'][number] : never
  : T extends JsonObject
  ? undefined extends T['properties'] 
    ? Record<string, unknown> 
    : T['required'] extends infer Required extends any[]
      ? Omit<{[key in (keyof T['properties'] & Required[number])]: JSONSchema2TS<T['properties'][key]>} & {[key in Exclude<keyof T['properties'], Required[number]>]?: JSONSchema2TS<T['properties'][key]>}, never>
      : {[key in keyof T['properties']]?: JSONSchema2TS<T['properties'][key]>}
  : T extends JsonArray
  ? undefined extends T['items'] ? unknown[] : JSONSchema2TS<T['items']>[]
  : never
