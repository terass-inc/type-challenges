...
type _A<
  ...
  N extends number,
  R extends any[] = []
> = M extends [infer H extends number[], ...infer T extends number[][]]
  ? _A<T, N, [...R, H[N]]>
  : R

type Transpose<M extends number[][], R extends any[] = []> = M extends [
  infer H1 extends number[],
  ...infer T1 extends number[][]
]
  ? H1 extends [infer H2 extends number, ...infer T2 extends number[]]
    ? Transpose<[T2, ...T1], [...R, H2]>
    : Transpose<T1, R>
  : R
