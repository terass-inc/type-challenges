...
type JSONSchema2TS<T> = T extends {enum: infer Enum extends any[]}
  ...
  : T extends {properties: infer Properties}
    ? T extends {required: infer Required extends any[]}
      ? Omit<
          {
            [k in keyof Pick<Properties, Required[number]>] : JSONSchema2TS<Properties[k]>
          }
          &
          {
            [k in keyof Omit<Properties, Required[number]>]? :  JSONSchema2TS<Properties[k]>
          },
          never
        >
      : {
