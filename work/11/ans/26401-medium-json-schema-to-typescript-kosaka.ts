...
type ParseObject<T> = T extends {
  ...
  ? T extends { required: infer R extends (keyof P)[] }
    ? Omit<
        {
          [_PK in keyof P as _PK extends R[number]
            ? _PK
            : never]: JSONSchema2TS<P[_PK]>
        } & { [_PK in keyof P]?: JSONSchema2TS<P[_PK]> },
        never
      >
    : { [_PK in keyof P]?: JSONSchema2TS<P[_PK]> }
  : Record<string, unknown>

type ParseArray<T> = T extends { items: infer I }
  ? JSONSchema2TS<I>[]
  : unknown[]

type JSONSchema2TS<T> = T extends {
  type: "object"
}
  ? ParseObject<T>
  ...
  ? E[number]
  : T extends { type: infer R extends keyof typeMap }
  ? typeMap[R]
  : never
