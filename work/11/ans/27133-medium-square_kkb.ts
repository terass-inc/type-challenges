...
type ABS<T extends NRange> = `${T}` extends `-${infer N extends NRange}` ? N : T
type fuga = ABS<-2>

type UnknownList<
  T extends NRange,
  U extends unknown[] = []
> = T extends U["length"] ? U : UnknownList<T, [unknown, ...U]>
...
type Square<N extends NRange> = Square_<ABS<N>, UnknownList<ABS<N>>>
