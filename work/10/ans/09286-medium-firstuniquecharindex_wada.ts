...
> = T extends `${infer F}${infer R}`
  ? F extends Char
    ? GetUniqueChar<R, Char, RepeatedChar | F>
    : GetUniqueChar<R, Char | F, RepeatedChar>
  : Exclude<Char, RepeatedChar>

type FirstUniqueCharIndex<
  T extends string,
  UniqueChar = GetUniqueChar<T>,
  current extends any[] = [],
> = T extends `${infer F}${infer R}`
  ? F extends UniqueChar
    ? current['length']
    : FirstUniqueCharIndex<R, UniqueChar, [...current, F]>
  : -1
