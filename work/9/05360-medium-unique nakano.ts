...
type IsInclude<T, V> = T extends [infer A, ...infer B]
  ? Equal<A, V> extends true
    ? true
    : IsInclude<B, V>
  : false
type Unique<T, Result extends any[] = []> = T extends [infer A, ...infer B]
  ? IsInclude<Result, A> extends true
    ? Unique<B, Result>
    : Unique<B, [...Result, A]>
  : Result
