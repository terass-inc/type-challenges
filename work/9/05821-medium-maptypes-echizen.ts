...
// 出来てない
type MapTypes<T, R extends MapObj> = {
  [P in keyof T]: T[P] extends R['mapFrom'] ? R['mapTo'] : T[P]
}

type T = MapTypes<{ date: string }, { mapFrom: string; mapTo: Date } | { mapFrom: string; mapTo: null }>
type T2 = MapTypes<{ name: string; date: Date }, { mapFrom: string; mapTo: boolean } | { mapFrom: Date; mapTo: string }>
