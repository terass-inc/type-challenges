...
type unknownList<T extends number, U extends unknown[] = []> = [unknown, ...U]['length'] extends T
  ? U
...
type LastIndexOf<
  T extends any[],
  U,
  Count extends unknown[] = unknownList<T['length']>
> = T extends [...infer TH, infer TT]
  ? Equal<U, TT> extends true
    ? Count['length']
    : Count extends [unknown, ...infer CT]
      ? LastIndexOf<TH, U, CT>
      : -1
  : -1
