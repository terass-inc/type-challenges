...
// Restがunknown[]になるのは何故？
type _TupleToNestedObject<T extends string[], U> = T extends [infer F, ...infer Rest]
  ? { [P in F]: TupleToNestedObject<Rest, U> }
  : U

type TupleToNestedObject<T extends any[], U> = T extends [infer F, ...infer Rest]
  ? { [P in F as P extends PropertyKey ? P : never]: TupleToNestedObject<Rest, U> }
  : U

type T = TupleToNestedObject<['a', 'b'], number>
type T2 = TupleToNestedObject<['a', 'b', 'c'], boolean>
